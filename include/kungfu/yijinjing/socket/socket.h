...
public:
  ...
  int bind(const struct sockaddr *addr, socklen_t addrlen) { return ::bind(this->sockfd(), addr, addrlen); }

  int connect(const struct sockaddr *addr, socklen_t addrlen) { return ::connect(this->sockfd(), addr, addrlen); }
  ...
  int send(const char *data, size_t length) const { return ::write(sock_, data, length); };
...
public:
  ...
  static socket_ptr udp_multicast_client(int listen_port, const std::string &local_address,
